...
import { CashRegister, Minus, Plus, Trash } from "@phosphor-icons/react/dist/ssr";
...
const Menu = () => {
  ...
  const updateQuantity = (nama, newQty) => {
    ...
    setAllOrders((prev) =>
      prev.map((item) =>
        item.nama === nama ? { ...item, qty: newQty } : item
      )
    );
  };
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <div className="lg:w-2/5 bg-white rounded-lg shadow-sm p-4 flex flex-col">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-xl font-bold flex items-center w-full gap-5"> <CashRegister size={30}/> {totalItem} Items</h2>
              <button
            ...
            <div className="mb-5 flex gap-5 items-center w-full text-sm">
              Pelanggan: 
              <input ref={nameRef} onChange={handleName} type="text" placeholder="Nama Pelanggan" className="bg-white border rounded-sm py-2 px-5 w-full"/>
            </div>
                ...
                <SelectContent>
                  {
                    tables.filter(table => table.status === "available").map(table => {
                      return (
                        <SelectItem key={table.table_number} value={table.table_number}>Meja { table.table_number }</SelectItem>
                      )
                    })
                  }
                </SelectContent>
                ...
                <Table className="w-full">
                  ...
                  <TableBody>
                    {allOrders.map((order, i) => (
                      <TableRow key={i}>
                        <TableCell className="capitalize font-medium text-left truncate max-w-[200px]">
                          {order.nama}
                        </TableCell>
                        ...
                        <TableCell className="text-center">
                          <div className="flex items-center justify-center gap-2">
                            <button
                              onClick={() => updateQuantity(order.nama, order.qty - 1)}
